...
local RussianwareFolder = "Russianware"
local ConfigurationFolder = RussianwareFolder.."/Configurations"
local ConfigurationExtension = ".rfld"
local settingsTable = {
	General = {
		-- if needs be in order just make getSetting(name)
		russianwareOpen = {Type = 'bind', Value = 'K', Name = 'Russianware Keybind'},
		-- buildwarnings
		-- russianwareprompts
...
local function loadSettings()
	...
	local success, result =	pcall(function()
		task.spawn(function()
			if isfolder and isfolder(RussianwareFolder) then
				if isfile and isfile(RussianwareFolder..'/settings'..ConfigurationExtension) then
					file = readfile(RussianwareFolder..'/settings'..ConfigurationExtension)
				end
			end
			...
			if useStudio then
				file = [[
		{"General":{"russianwareOpen":{"Value":"K","Type":"bind","Name":"Russianware Keybind","Element":{"HoldToInteract":false,"Ext":true,"Name":"Russianware Keybind","Set":null,"CallOnChange":true,"Callback":null,"CurrentKeybind":"K"}}},"System":{"usageAnalytics":{"Value":false,"Type":"toggle","Name":"Anonymised Analytics","Element":{"Ext":true,"Name":"Anonymised Analytics","Set":null,"CurrentValue":false,"Callback":null}}}}
	]]
			end
	...
	if not success then 
		if writefile then
			warn('Russianware had an issue accessing configuration saving capability.')
		end
	end
end
		...
		if useStudio then
			...
			task.spawn(function()
				...
				if success and reporter then
					pcall(function()
						reporter.report("Russianware", Release, InterfaceBuild)
					end)
				else
					warn("Failed to load or execute the reporter. \nPlease notify Russianware developers at sirius.menu/discord.")
				end
			end)
...
local Russianware = useStudio and script.Parent:FindFirstChild('Russianware') or game:GetObjects("rbxassetid://10804731440")[1]
...
repeat
	if Russianware:FindFirstChild('Build') and Russianware.Build.Value == InterfaceBuild then
		correctBuild = true
		break
	end
	...
	if not warned then
		warn('Russianware | Build Mismatch')
		print('Russianware may encounter issues as you are running an incompatible interface version ('.. ((Russianware:FindFirstChild('Build') and Russianware.Build.Value) or 'No Build') ..').\n\nThis version of Russianware is intended for interface build '..InterfaceBuild..'.')
		warned = true
	end

	toDestroy, Russianware = Russianware, useStudio and script.Parent:FindFirstChild('Russianware') or game:GetObjects("rbxassetid://10804731440")[1]
	if toDestroy and not useStudio then toDestroy:Destroy() end

	buildAttempts = buildAttempts + 1
until buildAttempts >= 2

Russianware.Enabled = false

if gethui then
	Russianware.Parent = gethui()
elseif syn and syn.protect_gui then 
	syn.protect_gui(Russianware)
	Russianware.Parent = CoreGui
elseif not useStudio and CoreGui:FindFirstChild("RobloxGui") then
	Russianware.Parent = CoreGui:FindFirstChild("RobloxGui")
...
if gethui then
	for _, Interface in ipairs(gethui():GetChildren()) do
		if Interface.Name == Russianware.Name and Interface ~= Russianware then
			Interface.Enabled = false
			Interface.Name = "Russianware-Old"
		end
	end
elseif not useStudio then
...
if Russianware.AbsoluteSize.X < minSize.X and Russianware.AbsoluteSize.Y < minSize.Y then
...
local Main = Russianware.Main
local MPrompt = Russianware:FindFirstChild('Prompt')
local Topbar = Main.Topbar
local Elements = Main.Elements
local LoadingFrame = Main.LoadingFrame
local TabList = Main.TabList
local dragBar = Russianware:FindFirstChild('Drag')
...
local Notifications = Russianware.Notifications

local SelectedTheme = RussianwareLibrary.Theme.Default

local function ChangeTheme(Theme)
	if typeof(Theme) == 'string' then
		SelectedTheme = RussianwareLibrary.Theme[Theme]
	elseif typeof(Theme) == 'table' then
		SelectedTheme = Theme
	end

	Russianware.Main.BackgroundColor3 = SelectedTheme.Background
	Russianware.Main.Topbar.BackgroundColor3 = SelectedTheme.Topbar
	Russianware.Main.Topbar.CornerRepair.BackgroundColor3 = SelectedTheme.Topbar
	Russianware.Main.Shadow.Image.ImageColor3 = SelectedTheme.Shadow

	Russianware.Main.Topbar.ChangeSize.ImageColor3 = SelectedTheme.TextColor
	Russianware.Main.Topbar.Hide.ImageColor3 = SelectedTheme.TextColor
	Russianware.Main.Topbar.Search.ImageColor3 = SelectedTheme.TextColor
	if Topbar:FindFirstChild('Settings') then
		Russianware.Main.Topbar.Settings.ImageColor3 = SelectedTheme.TextColor
		Russianware.Main.Topbar.Divider.BackgroundColor3 = SelectedTheme.ElementStroke
	end
	...
	for _, text in ipairs(Russianware:GetDescendants()) do
...
local function getAssetUri(id: any): string
	...
	elseif type(id) == "string" and not Icons then
		warn("Russianware | Cannot use Lucide icons as icons library is not loaded")
	else
		warn("Russianware | The icon argument must either be an icon ID (number) or a Lucide icon name (string)")
	end
	return assetUri
end
...
local function LoadConfiguration(Configuration)
	...
	if not success then warn('Russianware had an issue decoding the configuration file, please try delete the file and reopen Russianware.') return end

	-- Iterate through current UI elements' flags
	for FlagName, Flag in pairs(RussianwareLibrary.Flags) do
		...
		if (typeof(FlagValue) == 'boolean' and FlagValue == false) or FlagValue then
			...
			warn("Russianware | Unable to find '"..FlagName.. "' in the save file.")
			print("The error above may not be an issue if new elements have been added or not been set values.")
			--RussianwareLibrary:Notify({Title = "Russianware Flags", Content = "Russianware was unable to find '"..FlagName.. "' in the save file. Check sirius.menu/discord for help.", Image = 3944688398})
		end
	end
...
local function SaveConfiguration()
	...
	local Data = {}
	for i, v in pairs(RussianwareLibrary.Flags) do
...
function RussianwareLibrary:Notify(data) -- action e.g open messages
	task.spawn(function()
		...
		if data.Actions then
			warn('Russianware | Not seeing your actions in notifications?')
...
local function Hide(notify: boolean?)
	...
	if notify then
		if useMobilePrompt then 
			RussianwareLibrary:Notify({Title = "Interface Hidden", Content = "The interface has been hidden, you can unhide the interface by tapping 'Show Russianware'.", Duration = 7, Image = 4400697855})
		else
			RussianwareLibrary:Notify({Title = "Interface Hidden", Content = `The interface has been hidden, you can unhide the interface by tapping {settingsTable.General.russianwareOpen.Value or 'K'}.`, Duration = 7, Image = 4400697855})
		end
	end
...
local function updateSettings()
	...
	if success then
		...
		if writefile then
			writefile(RussianwareFolder..'/settings'..ConfigurationExtension, encoded)
		end
	end
end
...
local function createSettings(window)
	...
	local newTab = window:CreateTab('Russianware Settings', 0, true)

	if TabList['Russianware Settings'] then
		TabList['Russianware Settings'].LayoutOrder = 1000
	end

	if Elements['Russianware Settings'] then
		Elements['Russianware Settings'].LayoutOrder = 1000
	end
...
function RussianwareLibrary:CreateWindow(Settings)
	if Russianware:FindFirstChild('Loading') then
		if getgenv and not getgenv().russianwareCached then
			Russianware.Enabled = true
			Russianware.Loading.Visible = true

			task.wait(1.4)
			Russianware.Loading.Visible = false
		end
	end

	if getgenv then getgenv().russianwareCached = true end

	if not correctBuild and not Settings.DisableBuildWarnings then
		task.delay(3, 
			function() 
				RussianwareLibrary:Notify({Title = 'Build Mismatch', Content = 'Russianware may encounter issues as you are running an incompatible interface version ('.. ((Russianware:FindFirstChild('Build') and Russianware.Build.Value) or 'No Build') ..').\n\nThis version of Russianware is intended for interface build '..InterfaceBuild..'.\n\nTry rejoining and then run the script twice.', Image = 4335487866, Duration = 15})		
			end)
	end

	if isfolder and not isfolder(RussianwareFolder) then
		makefolder(RussianwareFolder)
	end
	...
	LoadingFrame.Title.Text = Settings.LoadingTitle or "Russianware"
	LoadingFrame.Subtitle.Text = Settings.LoadingSubtitle or "Interface Suite"

	if Settings.LoadingTitle ~= "Russianware Interface Suite" then
		LoadingFrame.Version.Text = "Russianware UI"
	end
	...
	if not Settings.DisableRussianwarePrompts then
		task.spawn(function()
			while true do
				task.wait(math.random(180, 600))
				RussianwareLibrary:Notify({
					Title = "Russianware Interface",
	...
	if Settings.Discord and not useStudio then
		if isfolder and not isfolder(RussianwareFolder.."/Discord Invites") then
			makefolder(RussianwareFolder.."/Discord Invites")
		end

		if isfile and not isfile(RussianwareFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension) then
			if request then
				pcall(function()
					request({
			...
			if Settings.Discord.RememberJoins then -- We do logic this way so if the developer changes this setting, the user still won't be prompted, only new users
				writefile(RussianwareFolder.."/Discord Invites".."/"..Settings.Discord.Invite..ConfigurationExtension,"Russianware RememberJoins is true for this invite, this invite will not ask you to join again")
			end
		end
	end
	...
	if (Settings.KeySystem) then
		...
		if isfolder and not isfolder(RussianwareFolder.."/Key System") then
			makefolder(RussianwareFolder.."/Key System")
		end
		...
		if Settings.KeySettings.GrabKeyFromSite then
			for i, Key in ipairs(Settings.KeySettings.Key) do
				...
				if not Success then
					print("Russianware | "..Key.." Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Russianware specific development.')
				end
			end
		end
		...
		if isfile and isfile(RussianwareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension) then
			for _, MKey in ipairs(Settings.KeySettings.Key) do
				if string.find(readfile(RussianwareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension), MKey) then
					Passthrough = true
				end
			end
		end
			...
			KeyUI.Main.Input.InputBox.FocusLost:Connect(function()
				...
				if KeyFound then 
					...
					if Settings.KeySettings.SaveKey then
						if writefile then
							writefile(RussianwareFolder.."/Key System".."/"..Settings.KeySettings.FileName..ConfigurationExtension, FoundKey)
						end
						RussianwareLibrary:Notify({Title = "Key System", Content = "The key for this script has been saved successfully.", Image = 3605522284})
					end
				else
	...
	function Window:CreateTab(Name, Image, Ext)
		...
		function Tab:CreateButton(ButtonSettings)
			...
			Button.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Russianware | "..ButtonSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Russianware specific development.')
					task.wait(0.5)
					Button.Title.Text = ButtonSettings.Name
		...
		function Tab:CreateColorPicker(ColorPickerSettings) -- by Throit
			...
			ColorPicker.Interact.MouseButton1Down:Connect(function()
				...
				if not opened then
					...
					TweenService:Create(Main, TweenInfo.new(0.2, Enum.EasingStyle.Exponential), {ImageTransparency = SelectedTheme ~= RussianwareLibrary.Theme.Default and 0.25 or 0.1}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and ColorPickerSettings.Flag then
					RussianwareLibrary.Flags[ColorPickerSettings.Flag] = ColorPickerSettings
				end
			end
			...
			Russianware.Main:GetPropertyChangedSignal('BackgroundColor3'):Connect(function()
				for _, rgbinput in ipairs(ColorPicker.RGB:GetChildren()) do
					if rgbinput:IsA("Frame") then
						rgbinput.BackgroundColor3 = SelectedTheme.InputBackground
						rgbinput.UIStroke.Color = SelectedTheme.InputStroke
		...
		function Tab:CreateInput(InputSettings)
			...
			Input.InputFrame.InputBox.FocusLost:Connect(function()
				...
				if not Success then
					...
					print("Russianware | "..InputSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Russianware specific development.')
					task.wait(0.5)
					Input.Title.Text = InputSettings.Name
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and InputSettings.Flag then
					RussianwareLibrary.Flags[InputSettings.Flag] = InputSettings
				end
			end
		...
		function Tab:CreateDropdown(DropdownSettings)
				...
				for _, Option in ipairs(DropdownSettings.Options) do
					...
					DropdownOption.Interact.MouseButton1Click:Connect(function()
						...
						if not Success then
							...
							print("Russianware | "..DropdownSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Russianware specific development.')
							task.wait(0.5)
							Dropdown.Title.Text = DropdownSettings.Name
							TweenService:Create(Dropdown, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and DropdownSettings.Flag then
					RussianwareLibrary.Flags[DropdownSettings.Flag] = DropdownSettings
				end
			end
		...
		function Tab:CreateKeybind(KeybindSettings)
				...
				elseif not KeybindSettings.CallOnChange and KeybindSettings.CurrentKeybind ~= nil and (input.KeyCode == Enum.KeyCode[KeybindSettings.CurrentKeybind] and not processed) then -- Test
					...
					if not KeybindSettings.HoldToInteract then
						...
						if not Success then
							...
							print("Russianware | "..KeybindSettings.Name.." Callback Error " ..tostring(Response))
							warn('Check docs.sirius.menu for help with Russianware specific development.')
							task.wait(0.5)
							Keybind.Title.Text = KeybindSettings.Name
							TweenService:Create(Keybind, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and KeybindSettings.Flag then
					RussianwareLibrary.Flags[KeybindSettings.Flag] = KeybindSettings
				end
			end
		...
		function Tab:CreateToggle(ToggleSettings)
			...
			if SelectedTheme ~= RussianwareLibrary.Theme.Default then
			...
			Toggle.Interact.MouseButton1Click:Connect(function()
				...
				if not Success then
					...
					print("Russianware | "..ToggleSettings.Name.." Callback Error " ..tostring(Response))
					warn('Check docs.sirius.menu for help with Russianware specific development.')
					task.wait(0.5)
					Toggle.Title.Text = ToggleSettings.Name
					TweenService:Create(Toggle, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if not ToggleSettings.Ext then
				if Settings.ConfigurationSaving then
					if Settings.ConfigurationSaving.Enabled and ToggleSettings.Flag then
						RussianwareLibrary.Flags[ToggleSettings.Flag] = ToggleSettings
					end
				end
			end
		...
		function Tab:CreateSlider(SliderSettings)
					...
					if SLDragging then
						...
						if SliderSettings.CurrentValue ~= NewValue then
							...
							if not Success then
								...
								print("Russianware | "..SliderSettings.Name.." Callback Error " ..tostring(Response))
								warn('Check docs.sirius.menu for help with Russianware specific development.')
								task.wait(0.5)
								Slider.Title.Text = SliderSettings.Name
								TweenService:Create(Slider, TweenInfo.new(0.6, Enum.EasingStyle.Exponential), {BackgroundColor3 = SelectedTheme.ElementBackground}):Play()
			...
			if Settings.ConfigurationSaving then
				if Settings.ConfigurationSaving.Enabled and SliderSettings.Flag then
					RussianwareLibrary.Flags[SliderSettings.Flag] = SliderSettings
				end
			end
	...
	function Window.ModifyTheme(NewTheme)
		...
		if not success then
			RussianwareLibrary:Notify({Title = 'Unable to Change Theme', Content = 'We are unable find a theme on file.', Image = 4400704299})
		else
			RussianwareLibrary:Notify({Title = 'Theme Changed', Content = 'Successfully changed theme to '..(typeof(NewTheme) == 'string' and NewTheme or 'Custom Theme')..'.', Image = 4483362748})
		end
	end
	...
	if not success then warn('Russianware had an issue creating settings.') end
...
function RussianwareLibrary:SetVisibility(visibility: boolean)
	setVisibility(visibility, false)
end

function RussianwareLibrary:IsVisible(): boolean
	return not Hidden
end

local hideHotkeyConnection -- Has to be initialized here since the connection is made later in the script
function RussianwareLibrary:Destroy()
...
if Topbar:FindFirstChild('Settings') then
	Topbar.Settings.MouseButton1Click:Connect(function()
		task.spawn(function()
			...
			Elements.UIPageLayout:JumpTo(Elements['Russianware Settings'])
		end)
	end)
...
hideHotkeyConnection = UserInputService.InputBegan:Connect(function(input, processed)
	if (input.KeyCode == Enum.KeyCode[settingsTable.General.russianwareOpen.Value or 'K'] and not processed) then
		if Debounce then return end
		if Hidden then
			Hidden = false
			Unhide()
...
function RussianwareLibrary:LoadConfiguration()
	local config
	...
	if CEnabled then
		...
		local success, result = pcall(function()
			...
			if isfile then 
				...
				notified = true
				RussianwareLibrary:Notify({Title = "Russianware Configurations", Content = "We couldn't enable Configuration Saving as you are not using software with filesystem support.", Image = 4384402990})
			end
		end)

		if success and loaded and not notified then
			RussianwareLibrary:Notify({Title = "Russianware Configurations", Content = "The configuration file for this script has been loaded from a previous session.", Image = 4384403532})
		elseif not success and not notified then
			warn('Russianware Configurations Error | '..tostring(result))
			RussianwareLibrary:Notify({Title = "Russianware Configurations", Content = "We've encountered an issue loading your configuration correctly.\n\nCheck the Developer Console for more information.", Image = 4384402990})
		end
	end
...
if useStudio then
	...
	local Window = RussianwareLibrary:CreateWindow({
		Name = "Russianware Example Window",
		LoadingTitle = "Russianware Interface Suite",
		Theme = 'Default',
		Icon = 0,
		...
		KeySettings = {
			...
			FileName = "Key", -- It is recommended to use something unique as other scripts using Russianware may overwrite your key file
			SaveKey = true, -- The user's key will be saved, but if you change the key, they will be unable to use your script
			GrabKeyFromSite = false, -- If this is true, set Key below to the RAW site you would like Russianware to get the key from
	...
	--RussianwareLibrary:Notify({Title = "Russianware Interface", Content = "Welcome to Russianware. These - are the brand new notification design for Russianware, with custom sizing and Russianware calculated wait times.", Image = 4483362458})
	...
	local thoptions = {}
	for themename, theme in pairs(RussianwareLibrary.Theme) do
...
task.delay(4, function()
	RussianwareLibrary.LoadConfiguration()
